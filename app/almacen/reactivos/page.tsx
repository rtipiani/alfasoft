...
export default function ReactivosPage() {
    ...
    // Subscribe to Inventory (Reactivos)
    useEffect(() => {
        const q = query(collection(db, "almacen_items"), where("tipo", "==", "REACTIVO"));
    ...
    useEffect(() => {
        const q = query(collection(db, "compras_requerimientos"), where("tipo", "==", "REACTIVO"));
        const unsub = onSnapshot(q, (snap) => {
    ...
    // Subscribe to Orders
    useEffect(() => {
        const q = query(collection(db, "compras_ordenes"), where("tipo", "==", "REACTIVO"));
        const unsub = onSnapshot(q, (snap) => {
    ...
    const handleSaveItem = async () => {
        try {
            ...
            setItemForm({
                ...
                categoria: "REACTIVO", grado: "", densidad: "", descripcion: "",
                stockMaximo: 0, puntoReorden: 0, costoUnitario: 0, ubicacion: "", lote: "", fechaVencimiento: ""
            });
            Swal.fire("Éxito", "Reactivo creado correctamente", "success");
        } catch (e: any) {
        ...
        try {
            if (editingReq) {
                ...
                await addDoc(collection(db, "compras_requerimientos"), {
                    tipo: "REACTIVO",
                    codigo: `REQ-${Date.now().toString().slice(-4)}`,
                    fecha: new Date(),
                    solicitante: reqForm.solicitante,
                    estado: "PENDIENTE",
                    items: [{
    ...
    const handleSaveOrder = async () => {
        try {
            await addDoc(collection(db, "compras_ordenes"), {
                tipo: "REACTIVO",
                codigo: `OC-REA-${Date.now().toString().slice(-4)}`,
                fecha: new Date(),
    ...
    const handleGenerateCode = () => {
        ...
        const code = `REA-${timestamp.slice(-4)}`;
        setItemForm({ ...itemForm, codigo: code });
    };
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400">Gestión de inventario y compras de reactivos químicos</p>
                </div>
            ...
            <div className="min-h-[400px]">
                ...
                {activeTab === "REQUERIMIENTOS" && (
                    <ProcurementTable
                        title="Requerimientos de Reactivos"
                        data={requirements}
                        type="REQUERIMIENTO"
                ...
                {activeTab === "ORDENES" && (
                    <div className="space-y-4">
                        <ProcurementTable
                            title="Órdenes de Compra - Reactivos"
                            data={orders}
                            type="ORDEN"
                            onView={(item) => {
            ...
            <Dialog isOpen={isItemDialogOpen} onClose={() => setIsItemDialogOpen(false)} title="Nuevo Reactivo" maxWidth="max-w-4xl">
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <FormField label="Código">
                                <div className="flex w-full items-center bg-gray-50 dark:bg-zinc-800/50 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 relative transition-all">
                                    <input
                                        type="text"
                                        value={itemForm.codigo}
                                        onChange={(e) => setItemForm({ ...itemForm, codigo: e.target.value })}
                                        className="flex-1 px-4 py-2.5 bg-transparent border-none focus:ring-0 text-gray-900 dark:text-white placeholder-gray-400 outline-none w-full"
                                        placeholder="REA-001"
                                    />
                                    <button
                                        type="button"
                                        onClick={handleGenerateCode}
                                        className="px-3 py-1.5 m-1 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition shadow-sm flex items-center justify-center shrink-0"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            <FormField label="Grado / Pureza">
                                <input
                                    type="text"
                                    value={itemForm.grado}
                                    onChange={(e) => setItemForm({ ...itemForm, grado: e.target.value })}
                                    className="w-full px-4 py-2.5 bg-gray-50 dark:bg-zinc-800/50"
                                    placeholder="Ej: 98%, PA, Técnico"
                                />
                            </FormField>
                            <FormField label="Densidad (g/ml)">
                                <input
                                    type="text"
                                    value={itemForm.densidad}
                                    onChange={(e) => setItemForm({ ...itemForm, densidad: e.target.value })}
                                    className="w-full px-4 py-2.5 bg-gray-50 dark:bg-zinc-800/50"
                                    placeholder="Ej: 1.84"
                                />
                            </FormField>
                        </div>
                        <FormField label="Descripción / Formula" className="mt-4">
                            <textarea
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                            <FormField label="Ubicación (Estante/Fila)">
                                <input
                                    type="text"
