...
import { collection, addDoc, onSnapshot, query, orderBy, limit, doc, setDoc, getDoc, runTransaction } from "firebase/firestore";
...
export default function TolvaFinosPage() {
    ...
    // Subscribe to Stock
    useEffect(() => {
        const unsub = onSnapshot(doc(db, "produccion_stats", "tolva_finos"), (doc) => {
            if (doc.exists()) {
    ...
    useEffect(() => {
        const q = query(collection(db, "produccion_tolva_finos"), orderBy("timestamp", "desc"), limit(10));
    ...
    const openDialog = (type: "entrada" | "salida") => {
        setDialogType(type);
        setFormData({ cantidad: 0, origenDestino: type === "entrada" ? "Chancado" : "Molienda" });
        setShowDialog(true);
    };
    ...
    const handleSave = async () => {
        ...
        try {
            await runTransaction(db, async (transaction) => {
                // 1. READS
                const statsRef = doc(db, "produccion_stats", "tolva_finos");
                ...
                // 3. WRITES
                // Create Log
                const newLogRef = doc(collection(db, "produccion_tolva_finos"));
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Tolva de Finos</h2>
                    <p className="text-gray-500 dark:text-gray-400">Almacenamiento de mineral triturado para molienda</p>
                </div>
            ...
            <div className="bg-white dark:bg-zinc-900 p-8 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 flex flex-col items-center justify-center">
                <div className="relative w-64 h-64">
                    ...
                    <div className="absolute inset-0 border-4 border-gray-300 dark:border-zinc-700 rounded-b-full border-t-0 flex items-end justify-center overflow-hidden">
                        <div
                            className="w-full bg-stone-400 dark:bg-stone-500 transition-all duration-1000"
                            style={{ height: `${Math.min((stock / 800) * 100, 100)}%` }}
                        ></div>
                    </div>
                    <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center">
                            <span className="text-4xl font-bold text-gray-900 dark:text-white">{stock.toFixed(2)}</span>
                            <p className="text-sm text-gray-500 dark:text-gray-400">Toneladas (TM)</p>
                        </div>
                    </div>
                </div>
                <p className="mt-4 text-sm text-gray-500">Capacidad Total: 800 TM</p>
            </div>

            {/* Production Metrics */}
            <ProductionMetrics defaultCapacidad={800} defaultJornada={12} />

            {/* Recent Movements Table */}
            <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 overflow-hidden">
                <div className="p-6 border-b border-gray-200 dark:border-zinc-800">
                    <h3 className="font-semibold text-gray-900 dark:text-white">Flujo de Material</h3>
                </div>
                <table className="w-full">
                    <thead className="bg-gray-50 dark:bg-zinc-800/50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Detalle</th>
                        </tr>
                    </thead>
                    ...
                    <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-2xl w-full max-w-md">
                        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-zinc-800">
                            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
                                {dialogType === 'entrada' ? 'Registrar Ingreso' : 'Registrar Alimentación'}
                            </h3>
                            <button
